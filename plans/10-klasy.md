...
5. **Własne klasy wyjątków** na stronie 4 jako pierwsze zastosowanie dziedziczenia — domykają notę „Zapowiedź — własne typy wyjątków” z rozdziału 8; przykład z jednym łańcuchem dziedziczenia (`BladWieku(ValueError)` → `BladZakresu(BladWieku)` z atrybutem i `super().__init__(komunikat)`; nazwa `BladZakresu` zgodna z notą w rozdziale 8 — ustalenie recenzji).
