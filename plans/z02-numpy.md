...
4. **Modernizacje NumPy 2.x:** `numpy.polynomial.Polynomial.fit()` zamiast `np.polyfit()`; `np.linalg.lstsq()` (domyślne `rcond` od NumPy 2.0); `np.unique(return_inverse=True)`; `Generator.spawn()` (od 1.25) dla niezależnych strumieni; `np.lib.format.open_memmap()` i `np.load(mmap_mode="r")`; `np.vectorize` pokazany jako wygoda, nie przyspieszenie.
