...
Moduł `functools` obejmuje jeszcze narzędzia wymagające pojęć z dalszych rozdziałów: `cached_property` (klasy) oraz `total_ordering` i `singledispatchmethod` (wybór implementacji według typu argumentu), które omawiamy w podrozdziale [Przeciążanie operatorów](../11-model-danych/operatory.md) rozdziału 11. W następnym podrozdziale przechodzimy do modułu `itertools`, który wobec funkcji generatorowych z rozdziału 6 pełni tę samą rolę, jaką `functools` pełni wobec dekoratorów.
