...
Rozdział domyka zapowiedzi z rozdziałów [6. Funkcje](../06-funkcje/index.md) — zwracanie `None` jako sygnału błędu było rozwiązaniem tymczasowym, a wartość z `return` w funkcji generatorowej trafia do wyjątku `StopIteration` — i [7. Moduły, pakiety i biblioteka standardowa](../07-moduly/index.md): instrukcja `assert` i opcja `-O`, mechanizm `sys.exit()`, testy pytest rozszerzone o `pytest.raises()`. Narzędzia z tego rozdziału będą obecne w każdym dalszym, poczynając od następnego, w którym program zacznie czytać i zapisywać pliki. <!-- TODO: link po powstaniu rozdziału o wejściu i wyjściu -->
