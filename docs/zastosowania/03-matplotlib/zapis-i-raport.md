...
Rysunki z tego rozdziału powstają z tablic NumPy. Biblioteka pandas z następnych rozdziałów ścieżki rysuje wykresy wprost z tabel jedną metodą, opisując osie nazwami kolumn — u podstaw leży ten sam Matplotlib, więc każdy zwrócony obiekt `Axes` można dopracować metodami z tego rozdziału. <!-- TODO: link po powstaniu rozdziału o pandas --> Biblioteka seaborn buduje na Matplotlib wykresy statystyczne — rozkłady z podziałem na grupy, macierze zależności, regresje — jednym wywołaniem i z domyślnym stylem dobranym do danych tabelarycznych; warto ją poznać po pandas, gdy same dane są już tabelą.
