...
`np.linalg.norm()` liczy długość wektora (domyślnie euklidesową). Odległości między wszystkimi parami punktów daje rozgłaszanie z podrozdziału [Tablice wielowymiarowe](tablice-wielowymiarowe.md): `miasta[:, None, :]` ma kształt `(5, 1, 2)`, `miasta[None, :, :]` — `(1, 5, 2)`, a ich różnica — `(5, 5, 2)`, czyli wektor różnicy dla każdej pary; norma wzdłuż ostatniej osi zwraca macierz odległości. Wypełnienie przekątnej nieskończonością wyklucza odległość punktu od siebie, więc `argmin(axis=1)` wskazuje najbliższego sąsiada każdego miasta. Ten sam wzorzec — para osi z `None`, agregacja wzdłuż ostatniej — pojawi się w uczeniu maszynowym przy [grupowaniu](../10-bez-nadzoru/grupowanie.md) punktów. Rozszerzoną algebrę liniową dostarcza `scipy.linalg`.
