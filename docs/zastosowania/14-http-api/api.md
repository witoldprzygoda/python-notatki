...
Serwer w `http.server` pokazał każdy element interfejsu, ale każdy trzeba było napisać ręcznie: rozbiór ścieżki, dekodowanie JSON-a, walidację pól, kody błędów. Framework FastAPI z następnego rozdziału robi to z definicji funkcji i typów, generuje dokumentację interfejsu i pozwala podłączyć warstwę danych z rozdziału 13 — z sesją na każde żądanie <!-- TODO: link po powstaniu rozdziału o FastAPI -->; projekt ścieżki połączy bazę, API i okno aplikacji <!-- TODO: link po powstaniu rozdziału o projekcie aplikacji -->. Klient httpx przyda się jeszcze w ścieżce automatyzacji, przy pobieraniu stron. <!-- TODO: link po powstaniu rozdziału o pobieraniu i parsowaniu stron -->
