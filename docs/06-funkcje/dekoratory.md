...
Dekorator łączy pojęcia wprowadzone w tym rozdziale: funkcja jest obiektem (podrozdział [Definiowanie funkcji](definiowanie-funkcji.md#funkcja-jako-obiekt)), można ją przekazać innej funkcji i zwrócić z funkcji, a funkcja zagnieżdżona zachowuje wiązania zmiennych wolnych z funkcji otaczającej jako domknięcie. **Dekorator** (ang. *decorator*) to funkcja, która przyjmuje funkcję i zwraca w jej miejsce inną funkcję — zwykle domknięcie wywołujące funkcję pierwotną i wykonujące dodatkowe działania przed nią lub po niej. Zapis z symbolem `@` nad definicją jest jedynie skrótem dla przypisania wyniku dekoratora nazwie funkcji. W tym podrozdziale przechodzimy tę drogę krok po kroku, bez nowych mechanizmów języka, tak by składnia `@` nie była niczym więcej niż wygodnym zapisem operacji, którą umiemy wykonać ręcznie.

## Od funkcji do dekoratora

### Funkcja przyjmująca i zwracająca funkcję

W podrozdziale [Funkcje jako obiekty](funkcje-jako-obiekty.md#funkcje-pierwszej-klasy) przekazywaliśmy funkcje jako argumenty, a w podrozdziale [Zasięg nazw i domknięcia](zasieg-nazw-i-domkniecia.md#domkniecia) zwracaliśmy je jako wyniki. Najprostsza funkcja przyjmująca funkcję wywołuje ją z podanym argumentem; najprostsza funkcja zwracająca funkcję oddaje otrzymany obiekt bez zmian:
...
!!! note "Wyrażenie po znaku @"
    ...
    w ostatniej sekcji tego podrozdziału.
...
Składnia dekoratora powraca w dalszych rozdziałach książki. Przy klasach spotkamy dekoratory `@property` (podrozdział [Reprezentacja, właściwości i hermetyzacja](../10-klasy/reprezentacja-i-wlasciwosci.md#wasciwosci-property)) oraz `@classmethod` i `@staticmethod` (podrozdział [Atrybuty klasy, metody klasowe i statyczne](../10-klasy/atrybuty-i-metody.md#metody-klasy-classmethod)), a przy klasach danych `@dataclass` (podrozdział [Klasy danych — dataclass, NamedTuple i Enum](../12-oop-zaawansowane/klasy-danych.md#dekorator-dataclass)); w podrozdziale [Moduł functools](../07-moduly/functools.md) — `functools.cache` i `functools.wraps`. Każdy z nich korzysta z mechanizmu opisanego w tym podrozdziale: przyjmuje obiekt zdefiniowany instrukcją `def` lub `class` i zwraca obiekt, który zostaje związany z tą samą nazwą.
