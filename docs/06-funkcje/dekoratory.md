...
Składnia dekoratora powraca w dalszych rozdziałach książki. Przy klasach spotkamy dekoratory `@property`, `@classmethod` i `@staticmethod`, a przy klasach danych `@dataclass`; w podrozdziale [Moduł functools](../07-moduly/functools.md) — `functools.cache` i `functools.wraps`. Każdy z nich korzysta z mechanizmu opisanego w tym podrozdziale: przyjmuje obiekt zdefiniowany instrukcją `def` lub `class` i zwraca obiekt, który zostaje związany z tą samą nazwą. <!-- TODO: linki po powstaniu rozdziału o klasach -->
