...
Składnia dekoratora powraca w dalszych rozdziałach książki. Przy klasach spotkamy dekoratory `@property` (podrozdział [Reprezentacja, właściwości i hermetyzacja](../10-klasy/reprezentacja-i-wlasciwosci.md#wasciwosci-property)) oraz `@classmethod` i `@staticmethod` (podrozdział [Atrybuty klasy, metody klasowe i statyczne](../10-klasy/atrybuty-i-metody.md#metody-klasy-classmethod)), a przy klasach danych `@dataclass`; w podrozdziale [Moduł functools](../07-moduly/functools.md) — `functools.cache` i `functools.wraps`. Każdy z nich korzysta z mechanizmu opisanego w tym podrozdziale: przyjmuje obiekt zdefiniowany instrukcją `def` lub `class` i zwraca obiekt, który zostaje związany z tą samą nazwą. <!-- TODO: link po powstaniu rozdziału o klasach danych -->
