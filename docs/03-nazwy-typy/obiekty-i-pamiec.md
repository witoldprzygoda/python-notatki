...
Choć zarządzanie czasem życia obiektów w Pythonie jest zautomatyzowane, istnieje słowo kluczowe `del` — można w ten sposób usuwać obiekty, elementy złożonych obiektów (np. list czy słowników — rozdział [5. Typy złożone](../05-typy-zlozone/index.md)), czy nawet definicje klas (rozdział [10. Klasy i obiekty](../10-klasy/index.md)). Składnia jest prosta: `del nazwa_usuwanego_elementu`. Warto pamiętać, że `del` usuwa **nazwę** (referencję), a nie bezpośrednio obiekt — obiekt zniknie z pamięci dopiero wtedy, gdy jego licznik referencji spadnie do zera (podrozdział [Cykl życia obiektu](../10-klasy/cykl-zycia-obiektu.md#licznik-referencji-i-sysgetrefcount) w rozdziale 10).
