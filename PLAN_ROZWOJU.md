...
| 15 | 15. Współbieżność — wątki, procesy i GIL | `docs/15-wspolbieznosc/` | średni | 4 | index + 4 strony, 768 linii | lab11 | ukończony |
| 16 | 16. Warsztat programisty i dalsza droga | `docs/16-warsztat/` | średni | 5 | index + 5 stron, 864 linii | — | ukończony |
| 17 | 17. Pandas i analiza danych | `docs/17-pandas/` | niski | 5 | index + 5 stron, ok. 800–900 linii | — (dodatek) | przeniesiony do części II |
| 18 | 18. Interfejs graficzny tkinter | `docs/18-tkinter/` | niski | 7 | index + 7 stron, ok. 1050–1150 linii | — (dodatek) | przeniesiony do części II |
...
**Cel dydaktyczny.** Czytelnik rozumie mechanizm importu i przestrzeń nazw modułu, zamienia skrypt w program z `main()`, argumentami wiersza poleceń (`sys.argv`, minimalny `argparse`) i kodem wyjścia, organizuje kod w pakiety regularne z importami absolutnymi i względnymi, układa projekt (układ płaski → `src/`, minimalny `pyproject.toml`, instalacja edytowalna w venv), pisze pierwsze testy jako funkcje z `assert` i uruchamia je najpierw bez frameworka, potem minimalnym pytest (lab3), poznaje `sys`, `collections` (`deque`, `Counter`, `defaultdict`, krótko `namedtuple`), `functools` (`cache`/`lru_cache`, `wraps`, `partial`, `reduce` jednym akapitem) oraz ograniczony rdzeń `itertools`.

**Główne tematy.**
- moduł jako plik i obiekt (`__name__`, `__doc__`, `__file__`, `dir()`), formy import/from/as, `__all__` krótko, PEP 8 (sortowanie narzędziem → 16), wykonanie przy imporcie i `sys.modules`; `reload` i `__pycache__` tylko nota
- `sys.path` z mechanizmem wyszukiwania (`-P` w nocie, `PYTHONPATH` jednym zdaniem; `sys.path.append()` wyłącznie jako demonstracja zmiany listy, nie sposób organizacji projektu), `site-packages` ↔ venv, pułapka kolizji nazw ze stdlib (komunikat 3.13+)
- `__name__ == "__main__"`, `main()`, `python -m` (moduł) — osobna strona; `sys.argv`, `argparse` 3.14 (jeden pozycyjny, jedna opcja, `--help`, błąd → kod 2; `suggest_on_error` tylko dla `choices`, kolor w nocie), kody wyjścia i `sys.exit` — osobna strona
- pakiety regularne z `__init__.py`, importy absolutne i względne `.`/`..`, `__main__.py` i `python -m pakiet`, nota o `__spec__`; pakiety przestrzeni nazw (PEP 420) krótko („w tej książce tworzymy pakiety regularne; Python obsługuje też pakiety przestrzeni nazw bez `__init__.py`”)
- struktura projektu: układ płaski → src layout, minimalny `pyproject.toml` (Hatchling jako backend w przykładzie; `[build-system]` jako konfiguracja narzędzi do zbudowania i instalacji projektu, `[project]`), `python -m pip install -e .`; `assert` (`AssertionError`, `-O` → 8) z przykładem `float`/`math.isclose`/`Decimal` z napisów (lab3, bez NumPy); testy jako funkcje `test_*` w `tests/` uruchamiane bez frameworka, potem minimalny pytest (wariant A+: instalacja w venv, `python -m pytest`, odnajdywanie `test_*.py`/`test_*`, PASS/FAIL, kod wyjścia); bez `raises` (→ 8), fixtures/parametrize/mock/coverage/CI (→ 16); bez Gita, `.gitignore`, GitHub
- biblioteka standardowa: mapa modułów (`sys.stdlib_module_names` bez zapisywania liczby; `re` jako wzmianka), `sys` (`getrecursionlimit()` wykonywalnie, `setrecursionlimit()` jako API bez zachęty do zwiększania), `collections`: `deque`, `Counter`, `defaultdict`, `namedtuple` krótko (funkcja fabryczna tworząca podklasę krotki; zapowiedź klas danych → 12); `OrderedDict` jedno zdanie; `ChainMap` pominięty
- `functools` (osobna strona): `cache`/`lru_cache`, `wraps` (stabilny efekt: nazwa, docstring, adnotacje, `__wrapped__`), `partial`, `reduce` jednym akapitem
- `itertools` (osobna strona; materiał opcjonalny zaznaczony w treści, nie w tytule): `count`, `cycle`/`repeat`, `islice`, `chain`, `pairwise`, `batched(strict=True)`, `accumulate`, `starmap` krótko, `groupby`, jeden fragment `product`/`permutations`/`combinations`; `zip_longest`, `takewhile`, `dropwhile`, `filterfalse`, `compress`, `tee` tylko wymienione

**Zależności od wcześniejszych rozdziałów.**
- 1 (pip, venv, site-packages, sys.path, requirements.txt)
- 2 (opcja -m, uruchamianie skryptu, input())
- 3 (from-import przy Decimal/Fraction, int(), pułapka == dla float, Decimal)
- 5 (lista, słownik get()/setdefault(), krotka, zip)
- 6 (main() z funkcji, lambda dla groupby, generatory i yield from, callable dla defaultdict, dekoratory i memoizacja dla functools, rekurencja i limit dla sys)

**Zapowiedzi i luki, które rozdział domyka.**
- 15 collections.deque
- 6 import/from/as/assert
- 9 komentarzy TODO w rozdz. 6 (docstring modułu, `__main__.`, limit w `sys`, memoizacja/`cache`, `wraps`, `reduce`/`partial`/`itertools`) i zdanie we wstępie rozdz. 6
...
**Podział na strony.** Szczegóły: `plans/07-moduly.md`.

| Plik | Etykieta nav / H1 | Zawartość |
|---|---|---|
| `07-moduly/index.md` | Wprowadzenie | Od pliku do projektu; odsyłacze do 1, 2 i 6; ---; ## W tym rozdziale (8 podstron; itertools oznaczony jako opcjonalny). Bez „Powiązanego laboratorium” i „Ściągi”. |
| `07-moduly/moduly-i-import.md` | Moduły i instrukcja import | Moduł jako plik i obiekt (docstring modułu); Formy import (`__all__` krótko, PEP 8); Przestrzeń nazw modułu (dir, `__file__`); Wykonywanie przy imporcie (sys.modules; nota reload/__pycache__); Ścieżka wyszukiwania modułów (sys.path, -P, ModuleNotFoundError); Pułapki (random.py, import cykliczny prozą). |
| `07-moduly/skrypt-jako-program.md` | Skrypt jako program | Ten sam plik jako moduł i program; `__name__` i `"__main__"` (dwa pliki, nie samoimport); strażnik; konwencja main(); python -m (moduł). |
| `07-moduly/argumenty-wiersza-polecen.md` | Argumenty wiersza poleceń | sys.argv; obsługa ręczna i --help z `__doc__`; argparse (prog/description, pozycyjny, -n/--liczba type=int, Namespace, --help „options:”, błąd → kod 2; nota 3.14: suggest_on_error dla choices, color, prog pod -m); kody wyjścia i sys.exit(main()). |
| `07-moduly/pakiety.md` | Pakiety | Katalog jako pakiet (pakiet importowany a dystrybucyjny; `geometria` z funkcjami zamiast K1..K3, drzewo .text); `__init__.py` i interfejs pakietu (`__all__`); importy absolutne; importy względne (`.`/`..`, błąd „attempted relative import”); python -m i `__main__.py` (nota `__spec__`); pakiety przestrzeni nazw krótko. |
| `07-moduly/struktura-projektu.md` | Struktura projektu i pierwsze testy | Od skryptu do projektu (układ płaski, sys.path[0]); assert (pułapka krotki, float/isclose/Decimal z napisów, round krótko); katalog tests (funkcje test_*, python -m tests.test_figury, kod 0/1; tests/__init__.py dla spójności); pytest minimalny (A+); układ src/ i pyproject.toml (Hatchling, [build-system], [project]); python -m pip install -e . i pip show; admonition „Układ z laboratorium 3”; bez Gita. |
| `07-moduly/biblioteka-standardowa.md` | Biblioteka standardowa i moduł collections | Moduły w zestawie (tabela: użyte w 1–7 / zapowiadane; re jako wzmianka; stdlib_module_names); moduł sys (getrecursionlimit wykonywalnie — domyka rekurencja.md:310; setrecursionlimit jako API); deque (domyka lista.md:487); Counter (po idiomie zliczania słownikiem); defaultdict (a get()/setdefault()); namedtuple krótko (funkcja fabryczna, podklasa krotki, zapowiedź klas danych); OrderedDict jednym zdaniem. |
| `07-moduly/functools.md` | Moduł functools | cache/lru_cache (fib z rozdz. 6, cache_info, haszowalność; domyka dekoratory.md:374, rekurencja.md:207); wraps (stabilny efekt; domyka dekoratory.md:443, :540); partial; reduce jednym akapitem (initial jako argument nazwany 3.14; domyka funkcje-jako-obiekty.md:431). |
| `07-moduly/itertools.md` | Moduł itertools | Opcjonalność w pierwszym akapicie; count/cycle/repeat z islice; islice/chain; pairwise/batched(strict); accumulate/starmap; groupby (wymaga sortowania); product/permutations/combinations w jednym fragmencie; „Dalsze narzędzia” (zip_longest, takewhile, dropwhile, filterfalse, compress, tee — wymienione), przepisy z dokumentacji i more-itertools; domyka funkcje-generatorowe.md:248. |

**Istotne materiały źródłowe.**
- PDF: PythonNotatki.txt 2841–3185 (s. 76–84: moduły, sys.path, import, __all__, __name__, reload, pakiety, importy względne, PEP 420); 2007–2020 (sys.argv); 2651–2681 (reduce); 2704–2707 (sys.__dict__ — pominięte)
- DOPISANE: Wyklad_05.txt sl. 3–13, 31–36, 40, 43, 46; lab3.txt sl. 1–3, 5–6; lab2.txt sl. 6–8 (baza refaktoryzacji); lab6.txt sl. 1–2 i Wyklad_04.txt sl. 21, 26, 28 (functools)
- DOPISANE: Wyklad_08.txt sl. 26–31, 40 (itertools); Wyklad_03.txt sl. 15, 22–23, 31, 43; Wyklad_01.txt sl. 32

**Modernizacje i korekty względem źródeł (Python 3.14).**
- test.py → narzedzia.py; re „w C” → _sqlite3; timeit.timeit() jako zegar → pominięte; ścieżki C:\tmp → neutralne z venv i Install Managerem; samoimport test.py → dwa pliki
- moduł imp usunięty (3.12); deepreload pominięte; URL PEP 420 na peps.python.org; wszystkie wydruki dir() odtworzyć na 3.14.7 (zestaw dunder zależy od wersji), nie kopiować z PDF; atrybuty `__package__`/`__loader__`/`__cached__` przestarzałe — nie nauczane
- SyntaxError „import * only allowed at module level”; „from pkg import * bez __all__ → nic” uściślić (nazwy z __init__ bez wiodącego podkreślenia są widoczne); opis sys.path[0] z lab3 („katalog uruchomienia”) skorygowany (katalog skryptu / '' / cwd przy -m)
- układ lab3 „src jako pakiet” zastąpiony src/<pakiet>/ z instalacją edytowalną (wariant laboratoryjny w admonition); pyproject (PEP 621, Hatchling); python -m pytest; pytest 9.1.1 na 3.14; goły pip → python -m pip; assert dodaj(2.5, 3.1) == 5.6 → math.isclose/Decimal z napisów
...
**Priorytet:** wysoki. **Szacunek rozmiaru:** index + 6 stron, ok. 800–900 linii. **Status:** ukończony — projekt stron zaakceptowany 13 IX 2026 (`plans/08-wyjatki.md`); wszystkie strony wraz z `index.md` ukończone, zrecenzowane i zweryfikowane 13 IX 2026 (1823 linie); zapowiedzi z rozdziałów 1–7 domknięte odsyłaczami 13 IX 2026.
...
**Priorytet:** średni. **Szacunek rozmiaru:** index + 4 strony, ok. 600–700 linii. **Status:** ukończony — 13 IX 2026 (realizacja autonomiczna według `plans/15-wspolbieznosc.md`; index + 4 strony, 768 linii; wyniki 3.14t z komputera autora); zapowiedzi z rozdziałów 8, 10, 11 i 13 domknięte; zdanie o `pickle` w rozdziale 9 złagodzone; zapowiedź do części II (tkinter) zarejestrowana.
...
**Priorytet:** średni. **Szacunek rozmiaru:** index + 5 stron, ok. 800–900 linii. **Status:** ukończony — 13 IX 2026 (realizacja autonomiczna według `plans/16-warsztat.md`; index + 5 stron, 864 linii); zapowiedzi z rozdziałów 6, 11, 12 i 13 domknięte; zrzut panelu Testing zarejestrowany w `ZRZUTY.md`; do potwierdzenia przez autora: podział na części I/II widoczny dotąd tylko w tekście, nie w nawigacji.
...
**Priorytet:** niski. **Szacunek rozmiaru:** index + 7 stron, ok. 1050–1150 linii. **Status:** przeniesiony do części II (decyzja autora 13 IX 2026, `plans/CZESC-II.md`); projekt zachowany jako punkt wyjścia. Zapowiedź z rozdziału 15 (`synchronizacja.md`, sekcja „Wątki a interfejs graficzny”): `after()` i kolejka w programie okienkowym — `<!-- TODO: link po powstaniu rozdziału o tkinter -->`.
...
12. 17-pandas — dodatek bez labu; wymaga pandas 3.x, plików danych i testów seaborn. Przeniesiony do części II (decyzja autora 13 IX 2026).
13. 18-tkinter — dodatek bez labu; największy nakład rekonstrukcji listingów i weryfikacji Tk 9. Przeniesiony do części II (decyzja autora 13 IX 2026).
...
10. Rozdział 7 (5 IX 2026, szczegóły w `plans/07-moduly.md`): index + 8 podrozdziałów, w tym osobne strony „Argumenty wiersza poleceń” i „Moduł functools”; testy w wariancie A+ (najpierw `assert` i funkcje `test_*` uruchamiane bez frameworka, potem minimalny pytest: instalacja w venv, `python -m pytest`, odnajdywanie `test_*.py`/`test_*`, PASS/FAIL, kod wyjścia; `pytest.raises` pozostaje w rozdz. 8, zaawansowany pytest w rozdz. 16; pytest nie jest zależnością wykonawczą projektu); `collections`: `deque`, `Counter`, `defaultdict`, `namedtuple` krótko (funkcja fabryczna tworząca podklasę krotki, bez dziedziczenia i MRO), `OrderedDict` jedno zdanie, `ChainMap` pominięty; `itertools` ograniczony do rdzenia (count, cycle/repeat, islice, chain, pairwise, batched ze strict, accumulate, groupby, jeden fragment kombinatoryki, starmap krótko; reszta wymieniona); układ płaski → src layout, minimalny `pyproject.toml` z Hatchling jako backendem przykładu (bez porównywania backendów), instalacja edytowalna; bez Gita, GitHub, `.gitignore`, CI; etykieta i H1 „Moduł itertools” bez dopisku „(dla dociekliwych)” — opcjonalność zaznaczana w treści i w index; strona „Biblioteka standardowa i moduł collections”; szacunki długości są orientacyjne i nie stanowią limitu, „do około 6 H2” jest wskazówką strukturalną.
